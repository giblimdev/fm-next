import NavEmul from '@/components/nav/NavEmul'
import React from 'react'

function page() {
  return (
    <div>
      <h1>PROTOCOLE DE TABLE</h1>
<p>
  Les tenues sont ordinairement suivies d'une agape obligatoire durant laquelle
  les FF. peuvent converser librement entre eux, sous les réserves d'usage, et
  au cours de laquelle sont portés des Toasts, dans un ordre précis, constituant
  le protocole à table. La meilleure disposition de la table en en « U ». Les
  branches verticales du « U » figurent les cc. du N et du M. et la branche
  horizontale l'or. Les places de l'Or. sont réservées aux PP.MM. et GG.OO.
  Provinciaux (à la g. du VM.) ou nationaux (à la d. du VM.) Les SS. Se place au
  bout de de leur C. respective, face aux VM., dont la place est au milieu de
  l'Or. Avec le PMI. à sa g., tous les autres FF. se placent à table comme ils
  le souhaitent, y compris les Ap. et les Cp. Lorsque la cérémonie a donné lieu
  à un Initiation, le nouvel initié, est placé exceptionnellement à la droite
  immédiate du VM. Pour les autres cérémonies (Passage, Élévation) le Cand.
  prend place sur les cc.
</p>

<p>
  Avant le début des Toasts protocolaires, (et après les graces) la tradition
  permet de « prendre le vin avec... ». Cela doit être fait avec décorum et
  retenue. L'usage veut alors que l'on ne puisse inviter à « prendre le vin »,
  un F. d'un grade ou fonction supérieur. Nul ne peut donc solliciter le VM., à
  ce titre, sauf le GM ou le GMP Il est d'usage que le VM souhaite entendre «
  quelques mots de réponse » aux toasts qui sont portés. Dans ce cas, les
  réponses doivent être brèves.
</p>
<p>
  Il est de même lorsque les SS sont amenés à porter des Toast aux FF.
  visiteurs ou aux LL. sœurs. Au cours du banquet, nul ne peut se lever de
  table, à l’exception du Maître des banquets et des aides qu'il aura désignés
  (généralement des Ap.). Il est interdit de fumer sauf autorisation du VM.
  Lorsque les coups sont donnés les FF. doivent faire silence et être prêts à se
  lever à l'invitation du VM. ou du PMI.
</p>

<h2>LES GRACES</h2>
<p></p>

<p><b>DC. </b>: Mes FF., le VM. !</p>
<p className="com">
  Le VM gagne sa place, à l'O., (Les FF applaudissent le VM pendant son
  trajet).
</p>
<p><b>VM </b>: - O - <b>1er S </b> : - O - <b> 2d S </b>: - O -</p>
<p className="com">Les FF. se lèvent, font silence et font le s. de Foi.</p>
<p className="vm">
  <b>VM. </b>: Remercions le G.A.D.L'U. des aliments que nous allons partager et que
  nous recevons de Sa Main avec reconnaissance.
</p>
<p>
  <b>PMI </b>: <span className="com"> (Seul) </span>-OO- « Ainsi soit-il ». <span className="com">Tous cessent le s.
  de Foi. Le VM s'assoit et les FF. l’imitent. Le banquet peut alors commencer
  et le VM choisira les moments opportuns pour porter les Toasts qui suivent.</span>
</p>
<h2>TOASTS OFFICIELS ET OBLIGATOIRES</h2>
<p><b>PMI </b>: - O - <span className="com">et se lève</span></p>
<p><b>1er S </b>: - O - <span className="com">et se lève</span></p>
<p><b>2d S </b>: - O - <span className="com">et se lève</span></p>
<p className="com">(Les FF. font silence)</p>
<p><b>PMI </b>: FF. SS., tout est-il chargé sur vos colonnes respectives ?</p>
<p className="com">
  Les SS s'assurent que tout est bien chargé, sinon ils font le nécessaire.
  <br />
  Puis ils répondent :
</p>
<p><b>1er S </b>: Tout est chargé à l'Occ.</p>
<p><b>2d S </b>: Tout est chargé au M.</p>
<p className="com">Le PMI et les SS s'assoient.</p>
<h3>1/ Au président de la république Française</h3>
<p><b>PMI </b>: - O -</p>
<p><b>1er S </b>: - O -</p>
<p><b>2d S </b>: - O -</p>
<p><b>PMI </b>: Mes FF., prêtez attention au VM.</p>
<p>
  <b>VM </b>: <span className="com">(se lève)</span> Mes FF., je vous prie de bien vouloir
  vous lever <br />
  <span className="com">(Tous se lèvent).</span><br />
  Portons un toast au Président de la République Française. <br />
  <span className="com">Tous portent le toast</span> <br />
  <b>Tous </b>: « Au Président de la République Française »
</p>
<h3>2/ Aux souverains et chef d'états...</h3>
<p><b>PMI </b>: - O -</p>
<p><b>1er S </b>: - O -</p>
<p><b>2d S </b>: - O -</p>
<p><b>PMI </b>: Mes FF., prêtez attention au VM.</p>
<p>
  <b>VM </b>: <span className="com">(se lève)</span> Mes FF., je vous prie de bien vouloir
  vous lever <br />
  <span className="com">(Tous se lèvent).</span> <br />
  Portons un toast aux Souverains et Chefs d’États qui protègent le FM. <br />
  <span className="com">Tous portent le toast</span>
</p>
<p>
  <b>Tous </b>: « Aux Souverains et Chefs d’États qui protègent le FM. » Puis se
  rassoient.
</p>
<h3>3/ Au T.R.G.M.</h3>
<p><b>PMI </b>: - O -</p>
<p><b>1er S </b>: - O -</p>
<p><b>2d S </b>: - O -</p>
<p><b>PMI </b>: Mes FF., prêtez attention au VM.</p>
<p>
  <b>VM </b>: <span className="com">(se lève)</span> Mes FF., je vous prie de bien vouloir
  vous lever <br />
  <span className="com">(Tous se lèvent).</span> <br />
  Portons un toast Au T.R.G.M. <br />
  <span className="com">Tous portent le toast </span><br />
  <b>Tous </b>: « Au T.R.G.M. ! » <br />
  <span className="com">puis se rassoient.</span> <br />
  Si T.R.G.M. est présent, il peut, s'il le souhaite, prononcer quelques mots de
  réponse au Toast en son honneur.
</p>
<h3>4/ A la GLNF</h3>
<p><b>PMI </b>: - O -</p>
<p><b>1er S </b>: - O -</p> 
<p><b>2d S </b>: - O -</p>
<p><b>PMI </b>: Mes FF., prêtez attention au VM.</p>
<p>
  <b>VM </b>: <span className="com">(se lève)</span> Mes FF., je vous prie de bien vouloir
  vous lever <br />
  <span className="com"
    >(Tous se lèvent sauf les Officiers de la GL. RF ou TRF).
  </span>
  <br />
  Portons un Toast à la GLNF, à ses Dignitaires et Grand Off. <br />
  <span className="com">Tous portent le toast</span> <br />
  <b>Tous </b>: « A la GLNF. ! » <br />
  <span className="com">Puis se rassoient.</span> <br />
  Le VM, si cela a été convenue préalablement, peut demander une réponse à un
  Off. de G.L.
</p>
<p className="com">Ceci se fait alors dans les conditions suivantes :</p>
<p><b>PMI </b>: - O -</p>
<p><b>1er S </b>: - O -</p>
<p><b>2d S </b>: - O -</p>
<p>
  <b>PMI </b>: Mes FF., Le VM. Souhaiterait entendre quelques mots de réponse au nom
  de la GLNF., de la part du RF. …....... (ou TRF.)
</p>
<p className="com">
  Le F. nommé se lève, prononce sa réponse. Les FF. applaudissent à la fin de la
  réponse.
</p>
<h3>5/ Au TRGMP et à la GLP.</h3>
<p><b>PMI </b>: - O - <b>1er S</b> : - O - <b>2d S</b> : - O -</p>
<p><b>PMI </b>: Mes FF, prêtez attention au VM.</p>
<p>
  <b>VM </b>: (se lève) Mes FF., je vous prie de bien vouloir vous lever <br />
  (Tous selèvent sauf les Officiers de la GLP.). <br />
  Portons un Toast au TRGM. Pro. et à la GLP de ….... <br />
  Tous portent le toast
</p>
<p><b>Tous </b>: «au TRGM. Pro. et à la GLP de …....»</p>
<p>
  Puis se rassoient. Le VM., si cela a été convenue préalablement, peut demander
  une réponse à un Off. de GLP. Ceci se fait alors dans les conditions suivantes
  :
</p>
<p><b>PMI </b>: - O - <b>1er S</b> : - O - <b>2d S</b> : - O - </p>
<p>
  <b>PMI </b>: Mes FF., Le VM. souhaiterait entendre quelques mots de réponse au nom
  de la GLP., de la part du TRF. …....... (RF ou TVF.)
</p>
<p>
  F. nommé se lève, prononce sa réponse. Les FF. applaudissent à la fin de la
  réponse.
</p>
<h3>6/ Au Cand. de la cérémonie</h3>
<p>Ce Toast est porté au Cand. à la cérémonie qui a précédé l'agape.</p>

<p>Il peut s'agir d'un nouvel initié ou d'un nouveau Cp. Ou M.</p>
<p><b>PMI </b>: - O - <b>1er S</b> : - O - <b>2d S</b> : - O -</p>

<p>
  <b>PMI </b>: Mes FF., Prêtez attention au VM. qui désire porter un Toast au Cand. à la
  cérémonie de ce soir.
</p>
<p className="com">
  VM se lève et prononce une courte allocution à l'intention du récipiendaire.
  A la fin de son intervention, il dit :
</p>
<p><b>VM </b>: Mes FF., je vous prie de bien vouloir vous lever.</p>
<p className="com">(Tous se lèvent).</p>
<p><b>Tous </b>: « A notre nouveau F. (ou Cp. Ou M.) »</p>
<p className="com">
  Pendant ce toast, le F. en question reste assis et applaudit, puis les FF. se
  rassoient.
</p>
<p className="com">
  Lorsqu'il s'agit du nouveau M. installé, c'est le PMI. qui prononce
  l'allocution et qui invite les FF. à se lever et porter le Toast. Il est
  d'usage que celui qui a été ainsi honoré réponde au Toast.
</p>
<p className="com">Il le fait à l'invitation du PMI.</p>
<p><b>PMI </b>: - O - <b>
  1er S</b> : - O - <b>2d S</b> : - O -</p>
<p>
  <b>PMI </b>: Mes FF., Le VM. souhaiterait entendre la réponse de notre nouveau F.
  (Cp M ou M installé)
</p>
<p className="com">
  F. …..... se lève (encouragé par les applaudissements des FF.) donne ses
  impressions et se rassoit, tandis que les FF. l'applaudissent à nouveau.
</p>
<h3>7/ Aux LL. Sœurs</h3>
<br/>
<p><b>PMI </b>: - O - <b>1er S </b>: - O - <b>2d S </b>: - O -</p>
<p><b>
  PMI </b>: Mes FF., je vous prie de prêtez attention à notre F. 1er S. qui va
  porter un Toast aux LL. Sœurs.
</p>
<p>
  1er S <span className="com">(Le 1er S. se lève, prononce une courte allocution) </span>: Mes FF., je vous
  prie de bien vouloir vous lever.
</p>
<br/>
<p>
  S'il y a, dans l'assemblée un F. appartenant à une L. sœur, le VM. Peut
  souhaiter l'entendre répondre à ce toast.
</p>
<p>En ce cas, il est procédé comme pour la réponse d'un cand.</p><br/>
<h3>8/ Aux FF. Visiteurs</h3>
<p><b>PMI. </b>: -O- <b>1er S. </b>: -O- <b>2d S. </b>: -O-</p>
<p>
  <b>PMI. </b>: Mes FF., je vous prie de prêter attention à notre F. 2d S. qui va
  porter un Toast aux FF. visiteurs.
</p>
<p>
  Le 2d S se lève, prononce une courte allocution, puis il dit : 2d S. : Mes
  FF., je vous prie de bien vouloir vous lever.
</p>
<p>Les FF. se lèvent. Aux FF. visiteurs !</p>
<p><b>Tous </b>: « Aux FF. visiteurs ! »</p>
<p>par les visiteurs qui sont restés assis.</p>
<p>Puis les FF. se rassoient.</p>
<p>
  Après avoir été préalablement prévenus, un ou plusieurs visiteurs peuvent être
  amenés à répondre, et chaque fois le PMI. procédera comme pour un L. Sœur. En
  ce cas, il est procédé comme pour la réponse d'un cand.
</p>
<p className="com">
  C'est deux derniers Toasts peuvent être regroupés et portés par l'un ou
  l'autre des SS.
</p>
<h1>TOAST DU TUILLEUR</h1>
<p><b>PMI </b>: - OO - <span className="com">en restant assis</span></p>
<p>
  Le T se lève et vient avec son verre se placer derrière le VM. Il place sa
  main d. sur l'ép. g. du VM. et, tenant son verre de la m. g., dit :
</p>
<p>
  <b>T </b>: <span className="com">(ou un F.)</span> A tous les MM. Pauvres et dans la détresse, qu’ils soient
  dispersés sur la surface de la terre, sur les mers (et dans les airs), en leur
  souhaitant un rapide soulagement de leur souffrance et un prompt retour dans
  leur pays natal, si tel est leur désir. <br/>
  <span className="com">Les FF. se lèvent et portent le Toast</span> <br />
  <b>Tous </b>: "À tous les M. pauvres et dans la détresse !" <br/>
  Ce toast termine le protocole de table
</p>
<div><NavEmul /></div>

    </div>
  )
}

export default page