import NavEmul from '@/components/nav/NavEmul'
import React from 'react'

function page() {
  return (
    <div>
      <h1>FERMETURE AU TROISIEME GRADE</h1>

<p>V.M. : -O-</p>
<p>1er S : -O-</p>
<p>2d S : -O-</p>
<p>VM : Mes FF. unissez-vous à moi pour fermer la L. au troisième Grade.</p>
<p className="com">
Le VM se lève et les FF. l’imitent, dans un même mouvement.</p>
<p><b>VM </b>: F. 2d S quel est le constant devoir de tout M. M.?</p>
<p>
<b>2d S </b>: <span className="com">(tourne la tête vers le VM)</span> De s'assurer que la L. est bien couv.
</p>
<p><b>VM </b>: veuillez faire accomplir ce devoir</p>
<p>
<b>2d S </b>: <span className="com">(tourne la tête vers le C)</span> F. C assurez-vous que la L. est bien
  couverte.
</p>
<p><b>C </b>: <span className="com">(Le C. va à la porte)</span> - OO - O -</p>
<p><b>T </b>: - OO - O -</p>
<p>
<b>C </b> <span className="com">(Il vient à la g. du 1er S., fait le p. se met à l'O. de MM. tourne la
  tête vers le 2d S.) </span> : F. 2d S. la loge est bien couverte <span className="com">(fait le s. p. recouvre
  et quitte l'Ordre)</span>
</p>
<p>
<b>2d S </b>: - OO - O - <span className="com">fait le pas, se met à l'O., tourne la tête vers le VM.</span> VM la
  L. est bien couverte. Fait le s. p., recouvre et quitte.
</p>
<p><b>VM </b>: F 1er S. quel est le second devoir ?</p>
<p><b>1er S </b>: De s'assurer que les FF. se tiennent à l'ordre de M. M.</p>
<p><b>VM </b>: A l'ordre mes FF. Au Troisième Grade !</p>
<p className="com">
  Les FF. Font le p. et se mettent à l'o. en se réglant sur le VM.
</p>
<p><b>VM </b>: F. 2d S d'où venez-vous ?</p>
<p>
<b>2d S </b>: De l'Oc., où nous sommes allés à la recherche des authentiques secret
  du M. M.
</p>
<p><b>VM </b>: F. 1er S les avez-vous retrouvés ?</p>
<p>
<b>1er S </b>: Non VM, mais nous apportons certains s. substitués que nous désirons
  soumettre à votre approbation.
</p>
<p><b>VM </b>: Que ces s. substitués me soient communiqués dans les formes !</p>
<p className="com">
  Le 1er S, à l'o. quitte sa chaire par la g. et remonte la col. Du N. Ves
  l'Or. <br />
  Pendant ce temps, le 2d S quitte sa chaire par la g., en étant toujours à
  l'O., et attendant le que 1er S passe a sa hauteur pour se mettre en en
  marche, en partant du pied g. sur la même ligne que le 1er S, le long de la
  col. Du M.
  <br />Ils avancent ainsi de quelque pas, puis s'arrêtent et se font face en
  même temps. <br />Les SS doivent alors se trouver à deux pas l'un de l'autre,
  face à face à l'o. <br />Le 2d S fait un pas, tend la m. d. au 1er S et
  donne l'att. de p. conduisant du 2ème ou 3ème Grade, puis, il élève la m. du
  1er S, se penche légèrement et donne, à voix basse le m. de p. <br />Le 2d S
  lâche la m. du 1er S et tous deux se remettent à l'ordre de MM. Puis, le 2d
  S fait un petit pas, se met à l'o. De Cp. et fait le s. D'H., de comp. et le
  s. p. puis recouvre et reste à l'o. de MM Il tend sa m. d. au 1er S. et donne
  les c.p.p. et les m. À voix basse.<br />
  Ils se désengagent, reviennent à l'o. de MM. Le 2d S salue le 1er S par le
  s.p. de M.M.
   , recouvre, revient à l'o. et, faisant demi-tour, retourne à sa
  chaire par le coté d., puis fait le p. et reste à l'o. Le 1er S. se met dans
  l'axe central du T., se tourne vers Le VM. toujours à l'o. .
</p>
<p><b>1er S </b>: VM daignez recevoir, de ma part, les s. substitués du MM.</p>
<p>
<b>VM </b>: F. 1er S je les recevrai avec plaisir et, pour l'instruction des FF.,
  vous prononcerez les m. à haute voix. </p>
  <p className="com">
  Le VM. toujours à l'o., quitte sa chaire par le coté g. et viens se placer à
  deux pas du 1er S et face à se dernier fait le p.<br />
  Le 1er S fait le p., tend sa m. d. au VM, lui donne l'Att. de p.
  conduisant du 2ème au 3ème Grade, élève la m. du VM, se penche légèrement, et
  dit le m. de p. à haute voix. Ils reviennent tous deux à l'o. de MM. Le 1er S.
  fait un autre p., se met à l'ordre de Cp. et exécute les s. d'H., de C., le s.
  p. et recouvre. Ils se met à l'ordre de M.M., tend sa m. d. au VM; et donne les
  c.p.p. ainsi que les m. à haute voix. <br />
  Ils se désengagent, se mettent à
  l'o. Le 1er S salue le VM par le s. p. de MM., recouvre et revient à l'o. Le
  VM et le 1er S font, ensemble, demi-tour et chacun regagne, toujours à l'o.,
  sa chaire par le côté d. là il font le p. et restent à l'o.
</p>
<p>
<b>VM </b>: Mes FF. les secrets substitues du M. M. m'ayant été communiqué dans les
  formes, moi, en ma qualité de VM de cette L. et en tant que tel, l'humble
  représentant du R. S., je les ratifie et confirme par mon approbation et
  déclare qu'il vous désigneront, vous et tous les M. M. de par l'Univers,
  jusqu'à ce que le temps ou les circonstances nous permettent de retrouver les
  authentiques.
</p>
<p className="com">
  Tout les FF., sauf le VM, s'inclinent et disent, en se réglant sur le PMI. :
</p>
<p><b>Tous </b>: Nous nous inclinons devant notre M. avec reconnaissance.</p>
<p><b>VM </b>: Toute reconnaissance au Très Haut.</p>
<p className="com">
  Tous les FF. font le G.S. ou S.R. en se réglant sur le VM et reviennent
  ensuite à l'o. de MM.
</p>
<p>
<b>VM </b>: F. 1er S, les travaux de ce Grade étant terminés, je vous commende de
  fermer la L.
</p>
<p><b>VM </b>: -OO-O- <span className="com">(de la m. g.)</span></p>
<p>
<b>1er S </b>: Mes FF. au nom du Très HAUT, et par ordre du VM. Je ferme...
  <span className="com"> (Il fait le s.p. de MM., et tous les FF. l’imitent simultanément)</span
  ><br />
  ... cette Loge de MM. -OO-O-
</p>
<p><b>2d S </b>: Les travaux sont donc fermés. -OO-O-</p>
<p><b>C </b>: <span className="com">(va à la porte)</span> -OO-O-</p>
<p><b>T </b>: -OO-O-, <span className="com">le C. reviens à sa place.</span></p>
<p className="com">Le 2d E change la pl. tr.</p>
<p className="com">Le PMI arrange l'Eq. et le C.</p>
<p className="com">Le VM s’assoit et invite les FF. à prendre place.</p>

<div><NavEmul /></div>
    </div>
  )
}

export default page