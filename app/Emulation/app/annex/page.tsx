import NavEmul from '@/components/nav/NavEmul'
import React from 'react'

function page() {
  return (
    <div>
      <h1>SUSPENSION et REPRISE DES TRAVAUX</h1>
<h2>SUSPENSION DES TRAVAUX</h2>

<p><b>VM </b> : (- 0 -) 1er S. : (- 0 -) 2d S. : (- 0 -)</p>
<p><b>VM</b> : A moi FF. SS. !</p>
<p className="com">Le VM. et les SS. Se lèvent tandis que les FF. Restent assis</p>
<p><b>VM </b>: F. 2d S. quelle heure est-il ?</p>
<p><b>2d S </b>: Midi, VM.</p>
<p><b>VM </b>: quel est votre devoir ?</p>
<p><b>2d S </b>: D’appeler les FF. du travail au repos.</p>
<p><b>VM </b>: Veuillez annoncer l'heure du repos.</p>
<p>
<b>2d S </b>: Mes FF. Par ordre du VM., suspendez vos travaux et prenez du repos.
  Demeurez à portée de voix afin de pouvoir revenir, en temps opportun, pour
  notre plus grand profits et plaisir.
</p>
<p><b>2d S </b>: (- 0 -) 1er S. : (- 0 -) VM. : (- 0 -)</p>
<p className="com">
  Le PMI ferme le VSL sans déplacer l'Eq. Et le C. <br />
  le 2d S dresse sa c. et éteint sa l. <br />
  Le 1er S abaisse sa C. et éteint sa l. <br />
  Le 2d E retourne la planche tracée
</p>

<h2>REPRISE DES TRAVAUX</h2>

<p><b>VM </b>: (- 0 -) 1er S. : (- 0 -) 2d S. : (- 0 -)</p>
<p><b>VM </b>: A moi FF. SS. !</p>
<p className="com">Le VM. et les SS. Se lèvent tandis que les FF. Restent assis</p>
<p><b>VM </b>: F. 2d S. quelle heure est-il ?</p>
<p><b>2d S </b>: Passé Midi, VM.</p>
<p><b>VM </b>: quel est votre devoir ?</p>
<p><b>2d S </b>: D’appeler les FF. du travail au repos.</p>
<p><b>VM </b>: Veuillez annoncer la reprise des travaux.</p>
<p>
<b>2d S </b>: Mes FF. Par ordre du VM., veuillez reprendre le travail et poursuivre
  votre tâche maçonnique.
</p>
<p><b>2d S </b>: -0- <b>1er S</b> : -0- <b>VM </b>: -0-</p>
<p className="com">
  Le 2d S abaisse sa C. <br />
  Le 1er S dresse sa C. <br />
  Le 2d E allume la L. des SS. et découvre la P.T. <br />
  Le PMI ouvre le VSL et ajuste l'Eq et le C. si nécessaire.
</p>

<h1>« PORTER » et « RAMENER » LES TRAVAUX</h1>
<p>
  Exceptionnellement, le plus souvent pour des raisons de temps le VM. Peut «
  porter » les travaux au grade supérieur ou les « ramener » les travaux au
  grade précédent.
</p>

<p><b>VM </b>: -O- ; <b>1er S</b> : -O- ; <b>2d S</b> : -O-</p>

<p>
<b>VM </b>: Mes FF. en vertus des pouvoir qui n’ont été conféré, Les travaux de la
  L. sont portés (ou ramenés) au … Grade.
</p>
<p className="com">Ils frappent les coups du grade</p>
<p><b>VM </b>: - ?- ; <b>1er S </b>: - ?- ; <b>2d S </b>: - ?- ;</p>

<p><b>C </b>: - ?- ; <b>T </b>: - ?-</p>
<p className="com">
  selon le grade. <br />
  Le 2dE. : Expose la planche adhoc, <br />le PMI. Ajuste la position de l’Eq.
  et du C.
</p>
<br />
<h2>ETUDE DES CANDIDATURES</h2>
<h3>DES CANDIDATS PREMIERE LECTURE</h3>
<br />
<p>
  Le Sec. Donne seulement les nom, prénom, âge, profession et adresse du Cand.
  et cite le nom du parrain. <br />
  Il n’y a aucun commentaire. <br />
  Le VM désigne au moins deux enquêteurs. <br />
  Le Cand. est reçus en comité de L. En fonction des rapports d’enquêtes et de
  l’audition du cand le comité décide des suites à donner à cette candidature
</p>

<h3>DEUXIEME LECTURE</h3>
<p>
  Le Sec : donne les mêmes indications que lors de la première lecture.
  <br />
  Le VM prie le parrain de présenter et de parler du Cand. Les enquêteurs
  présentent leurs conclusions
  <span className="com"
    >(les enquêtes peuvent être lus. Ce n’est pas une obligation)</span>
  <br />
  Il n’y aucun commentaire. <br />
  La candidature donne lieu alors aux votes par ballots. <br />
  Tous les FF. votent. <br />
</p>

<h2>LES VOTES</h2>
<p>
  Tous les FF. de la L. sans distinction de grade vote.<br />
  Pour les votes secret les FF. utilisent des boules noires et boules blanches.
  <br />

  Le 1er E. distribue à chaque F. une boule blanche et une boule noire en
  commençant par le PMI et en finissant par le VM. <br />
  Le 2d E. montre au VM et aux FF. que l’urne est vide
  <span className="com">(il monte à la gauche du VM.)</span>, puis il collecte les
  boules. <br />
  pour un vote favorable le F. aura pris soins de mette la boule blanche du
  côté blancs et la boule noire du côté noir
  <span className="com"> (l’inverse si son vote est défavorable)</span> Le 2d E.
  présente l’urne au VM. <span className="com">(et au PMI.)</span> les résultats
  sont annoncés immédiatement.
</p>

<p><b>VM </b>: Le vote est « Favorable »</p>
<p className='com'>il n’y aucun autre commentaire.</p>

<div><NavEmul /></div>
    </div>
  )
}

export default page